...
using EventCallback = void (*)(int, int16_t, void*);
inline static struct timeval ZeroSecond = {0, 0};
inline static struct timeval OneMicrosecond = {0, 1};

// use single ']' as separator
constexpr std::string_view kFailCurl = "Failed to curl]";
constexpr int kStats = 5;  // Stats log e.g. time, byte size, etc.
...
int OnLibcurlTimerUpdate(CURLM* multi, int64_t timeout_ms,
                         void* timer_event_arg) {
...
void MultiCurlRequestManager::OnLibeventSocketActivity(int fd, int16_t kind,
                                                       void* data) {
...
MultiCurlRequestManager::MultiCurlRequestManager(
    ...
    const int64_t curlmopt_max_host_connections, Executor& executor)
    : running_handles_(0),
...
// static
void MultiCurlRequestManager::ShutdownEventLoop(int fd, int16_t event_type,
                                                void* arg) {
  // Cancel all requests.
...
// static
void MultiCurlRequestManager::StartedEventLoop(int fd, int16_t event_type,
                                               void* arg) {
  auto* self = reinterpret_cast<MultiCurlRequestManager*>(arg);
...
// static
void MultiCurlRequestManager::OnProcessingStarted(int fd, int16_t event_type,
                                                  void* arg) {
...
void MultiCurlRequestManager::StartProcessing(
  ...
  //
  // Note: Rather than directly adding the event to multi-handle here we hop
...
// static
void MultiCurlRequestManager::MultiTimerCallback(int fd, int16_t what,
                                                 void* arg) {
  auto* self = reinterpret_cast<MultiCurlRequestManager*>(arg);
