...
#ifndef SRC_CPP_COMMUNICATION_BHTTP_UTILS_H_
#define SRC_CPP_COMMUNICATION_BHTTP_UTILS_H_
...
absl::StatusOr<std::string> SerializeProtoToBHttp(
  ...
  if (auto maybe_serialized = bhttp_msg.Serialize(); !maybe_serialized.ok()) {
    return absl::InternalError(maybe_serialized.status().message());
  } else {
...
#endif  // SRC_CPP_COMMUNICATION_BHTTP_UTILS_H_
