...
TEST(JournalUtilsTests, CreateCheckpointBlobName) {
  ...
  partition_name = std::make_shared<std::string>("partition_name");
  EXPECT_SUCCESS(
...
TEST(JournalUtilsTests, ExtractCheckpointId) {
  ...
  blob_name = std::make_shared<std::string>("blob_name");
  EXPECT_EQ(
      JournalUtils::ExtractCheckpointId(partition_name, blob_name,
                                        checkpoint_id),
      FailureExecutionResult(errors::SC_JOURNAL_SERVICE_INVALID_BLOB_NAME));
  ...
  blob_name = std::make_shared<std::string>("partition_name/");
  EXPECT_EQ(
      JournalUtils::ExtractCheckpointId(partition_name, blob_name,
                                        checkpoint_id),
      FailureExecutionResult(errors::SC_JOURNAL_SERVICE_INVALID_BLOB_NAME));

  blob_name = std::make_shared<std::string>("partition_name/dsadas");
  EXPECT_EQ(
      JournalUtils::ExtractCheckpointId(partition_name, blob_name,
                                        checkpoint_id),
      FailureExecutionResult(errors::SC_JOURNAL_SERVICE_INVALID_BLOB_NAME));
  ...
  blob_name = std::make_shared<std::string>(
      "partition_name/dsadas_00000000000000012345");
  EXPECT_EQ(
      JournalUtils::ExtractCheckpointId(partition_name, blob_name,
                                        checkpoint_id),
      FailureExecutionResult(errors::SC_JOURNAL_SERVICE_INVALID_BLOB_NAME));
  ...
  blob_name = std::make_shared<std::string>(
      "partition_name/journal_00000000000000012345");
  EXPECT_EQ(
      JournalUtils::ExtractCheckpointId(partition_name, blob_name,
                                        checkpoint_id),
      FailureExecutionResult(errors::SC_JOURNAL_SERVICE_INVALID_BLOB_NAME));
...
TEST(JournalUtilsTests, ExtractBlobNameId) {
  ...
  blob_name = std::make_shared<std::string>("partition_name/dsadas_12345");
  EXPECT_EQ(
      JournalUtils::ExtractBlobNameId(partition_name, blob_name,
                                      kJournalBlobNamePrefix,
                                      kJournalBlobNamePrefixLength, journal_id),
  ...
  blob_name = std::make_shared<std::string>("partition_name/journal_12345");
