 ...
 */
enum ErrorCode nsm_extend_pcr(int32_t fd, uint16_t index, const uint8_t* data,
                              uint32_t data_len, uint8_t* pcr_data,
 ...
 * *Returns*: The status of the operation.
 */
enum ErrorCode nsm_describe_pcr(int32_t fd, uint16_t index, bool* lock,
 ...
 * *Returns*: The status of the operation.
 */
enum ErrorCode nsm_get_description(int32_t fd,
                                   struct NsmDescription* nsm_description);
 ...
 * *Returns*: The status of the operation.
 */
enum ErrorCode nsm_get_attestation_doc(
    int32_t fd, const uint8_t* user_data, uint32_t user_data_len,
    const uint8_t* nonce_data, uint32_t nonce_len, const uint8_t* pub_key_data,
    uint32_t pub_key_len, uint8_t* att_doc_data, uint32_t* att_doc_len);

/**
 * NSM `GetRandom` operation for non-Rust callers. Returns up to 256 bytes of
 * random data. *fd (input)*: A valid descriptor to the NSM device. *buf
 * (output)*: A valid buffer to place the random data in. *buf_len (input /
 * output)*: The length of the passed buffer and the length of the output data
