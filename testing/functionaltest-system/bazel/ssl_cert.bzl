...
def generate_ca_bundle(name, certs, test_tags = []):
    """Generates a root CA bundle from a list of signed SSL certificates.

    Arguments:
        name (string): name of bundle target
        certs ([Label]): list of target certificates
        test_tags (list[string]): optional list of test tags
    """

    bundle_file = "{}.bundle".format(name)
    native.genrule(
        name = name,
        srcs = certs,
        outs = [bundle_file],
        cmd_bash = """
cat {} >'$@'
        """.format(" ".join(["$(execpath {})".format(c) for c in certs])),
        local = True,
        message = "generate a root CA bundle",
    )
    native.sh_test(
        name = "{}_test".format(name),
        data = [":{}".format(bundle_file)],
        size = "small",
        srcs = [Label("//bazel:verify_ca_bundle")],
        args = [
            "$(location :{})".format(bundle_file),
            str(len(certs)),
        ],
        tags = [
            "local",
