...
bool fetchAttestationReport5(const char* report_data_hexstring,
  ...
  // the report data is passed as a hexstring which needs to be decoded into an
  // array of unsigned bytes MAA expects a SHA-256. So we use left align the
  // bytes in the report data

  uint8_t* reportData =
      decodeHexString(report_data_hexstring, sizeof(msg_report_in.report_data));
  memcpy(msg_report_in.report_data, reportData,
         sizeof(msg_report_in.report_data));
