...
#include "src/cpp/util/status_macro/status_builder.h"
...
//   }
//
// The macro ends with a `::privacy_sandbox::server_common
...
//   }
//
// `::privacy_sandbox::server_common ::StatusBuilder` supports
...
// interfaces.  See `::privacy_sandbox::server_common ::TaskReturn`
// and `rpc::RpcSetStatus` for details.
//
//   void Read(StringPiece name, ::privacy_sandbox::server_common
//   ::Task* task) {
//     int64 id;
...
// to avoid confusion between a `::privacy_sandbox::server_common
...
#define PS_RETURN_IF_ERROR(expr)                                             \
  ...
  if (::privacy_sandbox::server_common ::status_macro_internal::             \
...
// well as a `::privacy_sandbox::server_common ::StatusBuilder` object
...
constexpr bool HasPotentialConditionalOperator(const char* lhs, int index) {
  return (index == -1
              ...
              : (lhs[index] == '?'
                     ? true
                     : ::privacy_sandbox::server_common ::internal::
...
#define PS_STATUS_MACROS_IMPL_PS_ASSIGN_OR_RETURN_3_(lhs, rexpr,               \
  ...
  PS_STATUS_MACROS_IMPL_PS_ASSIGN_OR_RETURN_(                                  \
      PS_STATUS_MACROS_IMPL_CONCAT_(_status_or_value, __LINE__), lhs, rexpr,   \
      ::privacy_sandbox::server_common ::StatusBuilder _(                      \
