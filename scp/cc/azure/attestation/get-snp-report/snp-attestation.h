...
typedef struct {
  uint32_t version;    // version no. of this attestation report. Set to 1 for
                       // this specification.
  ...
  uint64_t reported_tcb;  // Reported TCB version used to derive the VCEK that
                          // signed this report
  uint8_t reserved2[24];  // reserved
  uint8_t chip_id[64];    // Identifier unique to the chip
  uint8_t committed_svn[8];  // The current commited SVN of the firware (version
                             // 2 report feature)
  ...
  uint8_t
      signature[512];  // Signature of this attestation report. See table 23.
