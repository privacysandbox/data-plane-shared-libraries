...
# source.
source "amazon-ebs" "cpiovalidator" {
  ami_name      = "cpio-validator-${local.timestamp}"
...
build {
  sources = ["source.amazon-ebs.cpiovalidator"]
  ...
  provisioner "file" {
    source      = join("/", [var.distribution_dir, "/aws_nitro_enclaves_validator_image.eif"])
    destination = "/home/ec2-user/aws_nitro_enclaves_validator_image.eif"
  }
  provisioner "file" {
    source = join("/", [var.workspace, "google_internal/testing/ami/vsockproxy.service"])
    destination = "/home/ec2-user/vsockproxy.service"
  }
  provisioner "shell" {
    script = join("/", [var.workspace, "google_internal/testing/ami/amazonlinux2/setup"])
  }
  # Writes packer manifest to dist/aws/packer-manifest.json
  post-processor "manifest" {
    output = join("/", [var.distribution_dir, "/packer-manifest.json"])
    strip_path = true
  }
}
