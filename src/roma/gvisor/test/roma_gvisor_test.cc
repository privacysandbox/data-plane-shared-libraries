...
SampleResponse SendRequestAndGetResponse(
    GvisorSampleService<>& roma_service, std::string_view code_token,
...
TEST(RomaGvisorTest, LoadingBinaryInSandboxMode) {
  Mode mode = Mode::kModeGvisor;
...
TEST(RomaGvisorTest, LoadingBinaryInNonSandboxMode) {
  Mode mode = Mode::kModeLocal;
  GvisorSampleService<> roma_service = GetRomaService(mode, /*num_workers=*/1);
...
TEST(RomaGvisorTest, ExecuteMultipleCppBinariesInSandboxMode) {
...
TEST(RomaGvisorTest, ExecuteMultipleCppBinariesInNonSandboxMode) {
...
TEST(RomaGvisorTest, ExecuteCppBinaryUsingCallback) {
  GvisorSampleService<> roma_service = GetRomaService(Mode::kModeGvisor, 2);
...
TEST(RomaGvisorTest, ExecuteGoLangBinaryInSandboxMode) {
...
TEST(RomaGvisorTest, ExecuteCppBinaryWithCallbackInSandboxMode) {
  int64_t elem_size = 5;
  ...
  absl::StatusOr<GvisorSampleService<>> sample_interface =
      GvisorSampleService<>::Create(config);
  CHECK_OK(sample_interface);
