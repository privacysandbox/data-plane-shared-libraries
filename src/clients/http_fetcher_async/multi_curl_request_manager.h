...
#ifndef SRC_CLIENTS_HTTP_FETCHER_ASYNC_MULTI_CURL_REQUEST_MANAGER_H_
#define SRC_CLIENTS_HTTP_FETCHER_ASYNC_MULTI_CURL_REQUEST_MANAGER_H_
...
class MultiCurlRequestManager final {
 public:
  ...
  explicit MultiCurlRequestManager(const int64_t curlmopt_maxconnects,
                                   const int64_t curlmopt_max_total_connections,
                                   const int64_t curlmopt_max_host_connections,
                                   server_common::Executor& executor);

  // Cleans up the curl multi session. Please make sure all easy handles
 ...
 private:
  ...
  static void ShutdownEventLoop(int fd, int16_t event_type, void* arg);

  // Callback when the event loop is ready to start processing the request.
  static void OnProcessingStarted(int fd, int16_t event_type, void* arg);

  // This is the callback associated with the timer event which is an input
  // to `Configure` method above.
  static void MultiTimerCallback(int fd, int16_t what, void* arg);
  ...
  static void OnLibeventSocketActivity(int fd, int16_t kind, void* data);
...
#endif  // SRC_CLIENTS_HTTP_FETCHER_ASYNC_MULTI_CURL_REQUEST_MANAGER_H_
