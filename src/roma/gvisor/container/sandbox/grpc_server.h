...
class RomaGvisorServiceImpl final : public RomaGvisorService::CallbackService {
 public:
  ...
  grpc::ServerUnaryReactor* ExecuteBinary(
    ...
    if (!output.ok()) {
      reactor->Finish(
          privacy_sandbox::server_common::FromAbslStatus(output.status()));
      return reactor;
    }
  ...
  grpc::ServerUnaryReactor* LoadBinary(grpc::CallbackServerContext* context,
    ...
    if (!code_token.ok()) {
      reactor->Finish(
          privacy_sandbox::server_common::FromAbslStatus(code_token.status()));
      return reactor;
    }
  ...
  grpc::Status ExecuteBinaryBidiStreaming(
    ...
    absl::StatusOr<absl::Cord> output =
        pool_manager_->SendRequestAndGetResponseFromWorker(
            request.code_token(), request.serialized_request());
    if (!output.status().ok()) {
      return privacy_sandbox::server_common::FromAbslStatus(output.status());
    }
...
class RomaGvisorGrpcServer final {
 public:
  void Run(std::string_view server_socket,
           absl::Nonnull<RomaGvisorPoolManager*> pool_manager) {
